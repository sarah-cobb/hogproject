...
def roll_dice(num_rolls, dice=six_sided):
    ...
    n, dice_func = 0, 0
    pig_out= False
    num_rolls_func=num_rolls
    "dice = six_sided = make_fair_dice(6) which returns a random int 1-6"

    while(num_rolls_func !=0):
        dice_func = dice()
        if (dice_func == 1):
            pig_out= True
        else:
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    "score % 10 finds the remainder after dividing by 10"
    n=0
    if score<10:
        return 10
    else:
        n = score % 10
    return 10 - min(n , score // 10)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0< goal and score1< goal):
        if (player==0):
            ...
            if(abs(k-num_rolls) == 2 and feral_hogs==True):
                score0 += 3
            k=num_rolls
